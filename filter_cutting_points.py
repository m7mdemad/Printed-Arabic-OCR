...
def holeExist(word, boundaries, mti):
    ...
    img = word[0:mti, boundaries[1]:boundaries[0]]
    ...
    if dots:
        ...
        for j in range(word.shape[0]):
            if word[j][check] > 0 and flag == 0:
                count += 1
                flag = 1
            elif word[j][check] == 0 and flag == 1:
                flag = 0
        if count <= 1:
            return False
...
def get_SHP_boundaries(word, boundaries1, boundaries2, start, end):
    img = word[start:end, boundaries1[3]:boundaries2[3]]
    # cv2.imwrite('zmzm.png', img)
    hp_img = horizintal_projection(img)
    return np.sum(hp_img)
...
def isStroke(word, boundaries1, boundaries2, bl, top_pixel_in_word_index, mfv, mfv1, mti, second, skeleton, vp):
    ...
    if height_of_seg >= 2*second_peak or height_of_seg >= second_peak - 10:
        # print('cond3 checked: ', height_of_seg, second_peak)
    ...
    if mode_hp not in range(mfv - 600, mfv + 600):
        # print('cond4 checked', mode_hp, mfv, mfv1)
        return False

    # cond5:
    if holeExist(img, [img.shape[1], 0, int(img.shape[1]/2), int(img.shape[1]/2)], mti) or get_path_cost(skeleton, word, mti, boundaries1[1], boundaries1[0]) > 1000000000000000000 or vp[boundaries1[3]] == 0:
        # print('cond5 checked')
        return False
...
def dotsExistCount(word, boundaries1, boundaries2):
    img = word[0:word.shape[0], boundaries1[3] + 2:boundaries2[3] - 2]
    hp_img = horizintal_projection(img)
    cv2.imwrite('zfttttt.png', img)
...
def filter_cutting_points(skeleton, word, sr, baseLine, maxTransitionIndex, most_frequent_value, most_frequent_value_after_0, vp, hp, second):
    ...
    if i < len(sr) - 1 and isStroke(word, sr[i + 1], sr[i], baseLine, top_pixel_in_word_index, most_frequent_value,
        ...
        if holeExist(word, sr[i + 1], maxTransitionIndex):
            valid_sr.append(sr[i])
            i += 1

        elif i < len(sr) - 2 and isStroke(word, sr[i + 2], sr[i + 1], baseLine, top_pixel_in_word_index,
                                        most_frequent_value,
                                        most_frequent_value_after_0, maxTransitionIndex, second, skeleton,
    ...
    while i < len(sr) - 1:
        ...
        if vp[sr[i][3]] == 0:
            if i < len(sr) - 1 and isStroke(word, sr[i + 1], sr[i], baseLine, top_pixel_in_word_index,
                ...
                elif i < len(sr) - 2 and get_heighest_pixel_index(
                    horizintal_projection(word[0:word.shape[0], sr[i + 1][3]:sr[i][3]])) < maxTransitionIndex - 5 \
        ...
        # if no path exist
            ...
            # count = dotsExistCount(word, sr[i + 1], sr[i])
            # if i == len(sr) - 2 and count >= 2:
            #     print(count, 'warning', i)
            #     i += 1
            ...
            if i < len(sr) - 1 and isStroke(word, sr[i + 1], sr[i], baseLine, top_pixel_in_word_index,
                ...
                # print('path not detected', cost, i, len(sr))
                valid_sr.append(sr[i])
                i += 1
        ...
        # detect holes case like: "ص, ض, ف, ه, ط"
            ...
            elif (vp[sr[i][3]]/255 < most_frequent_value/255) or (vp[sr[i][3]] == 0 and most_frequent_value == 0) or (most_frequent_value == 0 and vp[sr[i][3]]/255 <= most_frequent_value_after_0/255):
                # print(vp[sr[i][3]], most_frequent_value)
                valid_sr.append(sr[i])
                i += 1
                
            elif not holeExist(word, sr[i-1], maxTransitionIndex) and ((vp[sr[i][3]]/255 <= most_frequent_value/255) or (vp[sr[i][3]] == 0 and most_frequent_value == 0) or (most_frequent_value == 0 and vp[sr[i][3]]/255 < most_frequent_value_after_0/255)):
                # print( vp[sr[i][3]], most_frequent_value)
                valid_sr.append(sr[i])
                i += 1

            elif i == 1 and ((vp[sr[i][3]]/255 <= most_frequent_value/255) or (vp[sr[i][3]] == 0 and most_frequent_value == 0) or (most_frequent_value == 0 and vp[sr[i][3]]/255 <= most_frequent_value_after_0/255)):
                # print( vp[sr[i][3]], most_frequent_value)
                valid_sr.append(sr[i])
                i += 1

            else:
                # print(most_frequent_value, vp[sr[i][3]], most_frequent_value_after_0)
                i += 1
            ...
            if get_SHP_boundaries(word, sr[i+1], sr[i], baseLine, word.shape[0]) > get_SHP_boundaries(word, sr[i+1], sr[i], 0, baseLine):
                valid_sr.append(sr[i])
                i += 1
            else:
                # print('skip1', i, get_highest_left_pixel(word, sr[i], sr[i + 1]), top_pixel_in_word_index, baseLine)
                i += 1
            ...
            # print('not stroke', i, vp[sr[i + 1][3]], most_frequent_value, baselineExist(word, sr[i + 1], baseLine))

            if (i != len(sr) - 3 and (vp[sr[i+1][3]] == 0 or get_path_cost(skeleton, word, maxTransitionIndex, sr[i+1][1], sr[i+1][0]) > 1000000000000000000
            ...
            or (i == len(sr) - 3 and top_pixel_in_word_index + 12 >= get_heighest_pixel_index(horizintal_projection(word[0:word.shape[0], sr[i + 1][3]:sr[i][3]])) and holeExist(word, [sr[i + 1][3], sr[i][3]], maxTransitionIndex)) \
            or (i == len(sr) - 3 and top_pixel_in_word_index + 10 >= get_heighest_pixel_index(horizintal_projection(word[0:word.shape[0], sr[i + 1][3]:sr[i][3]]))):
                valid_sr.append(sr[i])
                i += 1
            ...
            elif i < len(sr) - 2 and isStroke(word, sr[i + 2], sr[i + 1], baseLine, top_pixel_in_word_index, most_frequent_value,
